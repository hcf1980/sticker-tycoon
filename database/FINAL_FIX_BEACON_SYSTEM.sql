...
BEGIN
  ...
  RAISE NOTICE '✅ 步驟 1：已建立缺少的 beacon_devices 記錄';
END $$;

-- ===== 步驟 2：刪除所有舊版欄位 =====
DO $$ 
BEGIN
  -- 刪除 event_type（舊版，改用 trigger_type）
  IF EXISTS (
    SELECT 1 FROM information_schema.columns 
    WHERE table_name = 'beacon_actions' AND column_name = 'event_type'
  ) THEN
    -- 先遷移資料到 trigger_type（如果 trigger_type 已存在）
    IF EXISTS (
      SELECT 1 FROM information_schema.columns 
...
BEGIN
  ...
  RAISE NOTICE '✅ 步驟 3：已添加所有新版欄位';
END $$;

-- ===== 步驟 4：更新現有記錄的預設值 =====
UPDATE beacon_actions
SET 
  trigger_type = COALESCE(trigger_type, 'enter'),
...
BEGIN
  ...
  RAISE NOTICE '✅ 步驟 4-5：已更新預設值並設定 NOT NULL 約束';
END $$;
