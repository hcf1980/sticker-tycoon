...
SELECT 
  '🎯 診斷結果' as check_type,
  CASE 
    WHEN (SELECT COUNT(*) FROM beacon_devices WHERE hwid = '018d4b2f1dc' AND is_active = true) = 0 
      THEN '❌ 設備不存在或未啟用'
    WHEN (SELECT COUNT(*) FROM beacon_messages WHERE is_active = true) = 0 
      THEN '❌ 沒有訊息模板'
    WHEN (SELECT COUNT(*) FROM beacon_actions WHERE hwid = '018d4b2f1dc' AND is_active = true) = 0 
      THEN '❌ 沒有觸發動作（這是問題所在！）'
    WHEN (SELECT COUNT(*) FROM beacon_actions ba 
          LEFT JOIN beacon_messages bm ON ba.message_id = bm.id 
          WHERE ba.hwid = '018d4b2f1dc' AND ba.is_active = true AND (bm.id IS NULL OR bm.is_active = false)) > 0 
      THEN '⚠️ 有觸發動作，但訊息模板有問題'
    ELSE '✅ 配置正常，可能是其他問題'
  END as diagnosis,
  (SELECT COUNT(*) FROM beacon_devices WHERE hwid = '018d4b2f1dc' AND is_active = true) as device_count,
  (SELECT COUNT(*) FROM beacon_messages WHERE is_active = true) as message_count,
  (SELECT COUNT(*) FROM beacon_actions WHERE hwid = '018d4b2f1dc' AND is_active = true) as action_count;
