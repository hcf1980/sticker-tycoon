...
exports.handler = async (event, context) => {
  ...
  try {
    ...
    if (approved) {
      ...
      if (user) {
        ...
        if (tokenUpdateError) {
          ...
          console.log(`✅ 已發放 ${completionTokens} 張數給 ${user.display_name || application.line_id}`);
        }
      } else {
    ...
    return {
      ...
      body: JSON.stringify({
        success: true,
        message: approved ? '✅ 已批准影片並發放 250 張數' : '✅ 已拒絕影片'
      })
    };
