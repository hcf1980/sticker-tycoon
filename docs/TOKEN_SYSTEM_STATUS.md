...
- [x] `token_ledger` 表（代幣帳本，追蹤有效期和剩餘數量）
...
**支援操作：**
- ✅ 增加代幣（add）→ 同步更新 token_ledger
- ✅ 扣除代幣（deduct）→ 使用 FIFO 邏輯從 token_ledger 扣除
- ✅ 設定代幣（set）→ 計算差額後處理
...
5. 檢查 `users.sticker_credits` 是否已扣除過期代幣
...
### 1. 舊用戶資料遷移
如果有**現有用戶已經有代幣但沒有 token_ledger 記錄**，需要執行資料遷移：
...
| 任務 | 頻率 | SQL |
|------|------|-----|
| 標記過期代幣 | 每天 00:00 | `SELECT mark_expired_tokens();` |
