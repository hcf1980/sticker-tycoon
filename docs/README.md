# 📚 貼圖大亨 - 文檔索引

> 本目錄包含專案的技術文檔和功能說明。

---

## 📁 文檔結構

### 功能文檔

| 文檔 | 說明 |
|------|------|
| [DEMO_GALLERY_SUMMARY.md](./DEMO_GALLERY_SUMMARY.md) | 示範圖集功能完整說明 |
| [HOMEPAGE_GALLERY_FEATURE.md](./HOMEPAGE_GALLERY_FEATURE.md) | 首頁圖集功能 |
| [GRID_GENERATOR_GUIDE.md](./GRID_GENERATOR_GUIDE.md) | 網格生成器使用指南 |
| [TOKEN_SYSTEM_STATUS.md](./TOKEN_SYSTEM_STATUS.md) | 張數系統狀態說明 |

### 整合指南

| 文檔 | 說明 |
|------|------|
| [LINE_PAY_INTEGRATION_GUIDE.md](./LINE_PAY_INTEGRATION_GUIDE.md) | LINE Pay 支付整合指南 |

### 問題修復

| 文檔 | 說明 |
|------|------|
| [MIGRATION_FIX.md](./MIGRATION_FIX.md) | 資料庫遷移修復 |
| [TRIGGER_ERROR_FIX.md](./TRIGGER_ERROR_FIX.md) | 觸發器錯誤修復 |

---

## 🔗 其他重要文檔

- **[README.md](../README.md)** - 專案主說明文件
- **[TROUBLESHOOTING.md](../TROUBLESHOOTING.md)** - 問題診斷指南
- **[supabase-schema.sql](../supabase-schema.sql)** - 資料庫結構

---

## 📝 更新日誌

- **2025-01** - 清理重複文檔，整合文檔結構
- **2024-01** - 初始文檔建立



